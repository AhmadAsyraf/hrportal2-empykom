...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['staff_id']) && isset($_POST['or_title']) && isset($_POST['or_category'])) {
        // Retrieve form data
        $staff_id = $_POST['staff_id'];
        $title = $_POST['or_title'];
        $category = $_POST['or_category'];
       ...
       include('../dbconn.php');
        ...
        if (!empty($_FILES['or_doc']['name'])) {
            // File upload handling
            $target_dir = "../uploads"; // Directory where the file will be saved
            $target_file = $target_dir . basename($_FILES["or_doc"]["name"]);

            // Attempt to move the uploaded file to the target directory
            if (move_uploaded_file($_FILES["or_doc"]["tmp_name"], $target_file)) {
                // File upload successful, now insert data into database
                $document_name = basename($_FILES["or_doc"]["name"]);
            } else {
        ...
        $sql = "INSERT INTO orga (staff_id, or_title, or_category, or_doc ) VALUES (?, ?, ?, ? )";
    ...
    header("Location: ../model/update_orga.php");
    exit;
}
?>
